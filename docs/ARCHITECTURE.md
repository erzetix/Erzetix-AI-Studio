...
Параметры идентичности размещаются в защищённом хранилище, подсистема оперирует ссылками на них.
...
Утверждение материалов человеком. Реализован средствами дашборда клиента. Поддерживает три режима: ручной, гибридный и автономный. Режим устанавливается отдельно для каждого клиента или персонажа.

Материалы, исчерпавшие число циклов автоматической доработки, передаются в контур с заключением `escalated`.

### Модуль публикации

Формирует итоговый пакет утверждённого контента для передачи специалистам, размещающим материалы на платформах. Обеспечивает соблюдение требований к маркировке контента, созданного с использованием технологий искусственного интеллекта. Предоставляет интерфейс ввода показателей эффективности размещённых публикаций.

На текущем этапе размещение выполняется вручную, без прямой интеграции с API платформ.

### Агент-аналитик метрик

Обрабатывает показатели эффективности публикаций. Передаёт результаты анализа в ядро персонажа для корректировки поведенческой логики и в систему A/B-тестирования.
...
Единственная подсистема, с которой клиент взаимодействует напрямую. Обеспечивает формирование брифов, просмотр материалов и метрик, утверждение контента, формирование отчётов. Поддерживает режим гостевого входа для демонстрационных целей.

Документация: [`dashboard/`](../dashboard/)
...
К защищаемым сведениям относятся параметры идентичности персонажей (Character DNA, Facial DNA, Body DNA, Style DNA, Prompt DNA), системные промпты подсистем, шаблоны технических промптов, правила допустимого контента, требования брендов клиентов и учётные данные доступа к внешним сервисам.
