...
|---|---|---|---|
| `Brief` | Дашборд клиента | Ядро персонажа | Постановка задачи на кампанию |
...
| `ReviewReport` | Агент-хранитель бренда | API-оркестратор, дашборд клиента | Заключения по материалам |
| `RevisionNote` | Агент-хранитель бренда | Агент-креатор, агент-визуализатор | Замечание к доработке материала |
| `MaterialView` | Дашборд клиента | Интерфейс пользователя | Представление материала в контуре утверждения |
| `ApprovalDecision` | Дашборд клиента | API-оркестратор | Решение об утверждении или отклонении |
| `PublicationMetrics` | Модуль публикации | Агент-аналитик метрик | Показатели эффективности публикации |
| `CampaignRun` | API-оркестратор | Дашборд клиента | Состояние кампании и журнал выполнения этапов |

## Порядок обработки

### Постановка задачи

Дашборд клиента формирует объект `Brief` и передаёт его API-оркестратору. Оркестратор создаёт объект `CampaignRun` в состоянии `received` и инициирует обработку.
...
Материалы с заключением `approved` преобразуются дашбордом в объекты `MaterialView` и отображаются в контуре утверждения.

Решение пользователя передаётся API-оркестратору в виде объекта `ApprovalDecision`. Утверждение переводит кампанию в состояние `approved`, отклонение возвращает кампанию в состояние `drafting`.

### Публикация и обратная связь

Утверждённые материалы передаются в модуль публикации. После размещения специалисты по производству контента вводят показатели эффективности, формируется объект `PublicationMetrics`.
